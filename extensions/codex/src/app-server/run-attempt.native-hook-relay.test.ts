...
describe("runCodexAppServerAttempt native hook relay", () => {
  ...
  it("installs only the adopted route's frozen hook surface on a cold resume", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const firstHarness = createStartedThreadHarness();
    const firstRun = runCodexAppServerAttempt(createParams(sessionFile, workspaceDir), {
    ...
    await firstHarness.waitForMethod("turn/start");
    const firstRelayId = extractRelayIdFromThreadRequest(
      firstHarness.requests.find((request) => request.method === "thread/start")?.params,
    );
    await firstHarness.completeTurn({ threadId: "thread-1", turnId: "turn-1" });
    await firstRun;
    ...
    secondParams.runId = "run-2";
    const secondRun = runCodexAppServerAttempt(secondParams, {
      nativeHookRelay: { enabled: true, events: ["pre_tool_use", "permission_request"] },
    });
    await secondHarness.waitForMethod("turn/start");
    ...
    const resumeConfig = (resumeRequest?.params as { config?: Record<string, unknown> } | undefined)
      ?.config;
    expect(Array.isArray(resumeConfig?.["hooks.PreToolUse"])).toBe(true);
    expect(resumeConfig).not.toHaveProperty("hooks.PermissionRequest");
    await expect(
      invokeNativeHookRelay({
        provider: "codex",
        relayId: firstRelayId,
        event: "permission_request",
        rawPayload: { hook_event_name: "PermissionRequest", tool_name: "Bash", tool_input: {} },
      }),
